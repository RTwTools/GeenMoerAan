...
MoveHandler::MoveHandler(ros::Publisher *_publisher,moveit::planning_interface::MoveGroup *_group,tf::TransformListener *_listener)
...
bool MoveHandler::Move(geometry_msgs::Pose pose)  {
...
void MoveHandler::set_Path(const nav_msgs::Path path){

		Cordinates=path;
}
	nav_msgs::Path MoveHandler::get_Path() const{

		return Cordinates;}

void MoveHandler::path_CallBack(const nav_msgs::Path &path){
...
geometry_msgs::Pose MoveHandler::current_Pose_CallBack(){ 
			geometry_msgs::Pose tempPose;
			tf::StampedTransform transform;
			ros::Time now=ros::Time::now();
	if(	listener->waitForTransform("map","base_link",now,ros::Duration(2.0))){	ROS_INFO_STREAM(" found");
